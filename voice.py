...
def initialize():
    ...
    if engine is None and recognizer is None:
        ...
        engine.setProperty('voice', voices[0].id) #0 for Male (en), 1 for Female (en), 2 for Female (jp)
    ...
    try:
        with suppress_stdout():
            user_input = recognizer.recognize_google(
                audio, language="id-ID", show_all=False #Dialect control use id-ID
            )
        return user_input
...
def generate_response(prompt):
    ...
    response_text = response_text.split("Chatbot:", 1)[0]
...
def main():
    ...
    user_name = "Me"
    while True:
        # Get Audio From User and Change to Text
        audio = get_audio_input()
        user_input = recognize_speech(audio)

        # Disini bisa di customize lah
